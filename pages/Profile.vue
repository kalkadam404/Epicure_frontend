  ...
  <div class="max-w-6xl mx-auto px-4 py-10">
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div class="bg-white rounded-2xl shadow-lg overflow-hidden">
          <div class="p-8 flex flex-col items-center">
            <div class="relative mb-5">
              <div v-if="user.image_url" class="w-32 h-32 rounded-full overflow-hidden border-4 border-gray-100 shadow-md">
                <img :src="user.image_url" alt="Аватар пользователя" class="w-full h-full object-cover" />
              </div>
              <div v-else class="w-32 h-32 rounded-full overflow-hidden bg-gray-200 flex items-center justify-center border-4 border-gray-100 shadow-md">
                <span class="text-5xl text-gray-500">{{ userInitial }}</span>
              </div>
              
              <!-- Кнопка загрузки фото -->
              <label for="avatar-upload" class="absolute bottom-0 right-0 bg-black rounded-full p-2 shadow-md cursor-pointer hover:bg-gray-800 transition-colors">
                <input 
                  type="file" 
                  id="avatar-upload" 
                  accept="image/*" 
      ...
      <div class="md:col-span-2" data-aos="fade-up">
        ...
        <!-- Личная информация -->
        <div v-if="activeTab === 'personal'" class="bg-white rounded-2xl shadow-lg p-8">
          <h3 class="text-xl font-bold text-gray-800 mb-6">Личная информация</h3>
          
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            <div>
              <label class="block text-sm font-medium text-gray-500 mb-2">Имя пользователя</label>
            ...
            <div>
              ...
              <div class="text-lg text-gray-800">{{ user.email || 'Не указан' }}</div>
            </div>
            
            <div>
              <label class="block text-sm font-medium text-gray-500 mb-2">Язык</label>
        ...
        <!-- История заказов -->
        <div v-else-if="activeTab === 'orders'" class="bg-white rounded-2xl shadow-lg p-8">
          ...
          <div class="text-center py-8 text-gray-500">
            ...
            <button class="mt-4 px-6 py-2 bg-black text-white font-medium rounded-lg hover:bg-gray-800 transition-colors">
              Перейти в меню
            </button>
          </div>
        </div>
        ...
        <!-- Избранное -->
        <div v-else-if="activeTab === 'favorites'" class="bg-white rounded-2xl shadow-lg p-8">
    ...
    <div v-if="showEditProfileModal" class="fixed inset-0 flex items-center justify-center z-50 pointer-events-none">
      <div class="bg-white rounded-lg max-w-md w-full shadow-xl p-6 relative overflow-y-auto max-h-[90vh] pointer-events-auto border border-gray-200">
        <button @click="showEditProfileModal = false" class="absolute top-4 right-4 text-gray-600 hover:text-gray-900">
          <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
          </svg>
        </button>
        
        <h2 class="text-xl font-bold text-gray-800 mb-6">Редактировать профиль</h2>
          ...
          <div class="space-y-4">
            ...
            <div>
              ...
              <select 
                ...
                class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-black"
              >
                <option v-for="(name, code) in languageNames" :key="code" :value="code">{{ name }}</option>
              </select>
            </div>
          </div>
    ...
    <div v-if="showChangePasswordModal" class="fixed inset-0 flex items-center justify-center z-50 pointer-events-none">
      <div class="bg-white rounded-lg max-w-md w-full shadow-xl p-6 relative pointer-events-auto border border-gray-200">
        <button @click="showChangePasswordModal = false" class="absolute top-4 right-4 text-gray-600 hover:text-gray-900">
          <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
          </svg>
        </button>
    ...
    >
      <svg v-if="notification.type === 'success'" class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
